...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method after;
        try {
            before = pageClass.getMethod("before", HttpServletRequest.class, Map.class);
            method = pageClass.getMethod(route.getAction(), HttpServletRequest.class, Map.class);
            after = pageClass.getMethod("after", HttpServletRequest.class, Map.class);
        ...
        try (AfterMethod ignored = () -> invoke(after, page, request, view)) {
            invoke(before, page, request, view);
            invoke(method, page, request, view);
    ...
    private void invoke(Method method,
                        Object page,
                        HttpServletRequest request,
                        Map<String, Object> view) throws InvocationTargetException, ServletException {
        try {
            method.setAccessible(true);
            method.invoke(page, request, view);
        } catch (IllegalAccessException e) {
            throw new ServletException("Unable to run action [pageClass=" + method.getDeclaringClass() + ", method=" + method + "].", e);
        }
    }

    private interface AfterMethod extends AutoCloseable {
        @Override
        void close() throws InvocationTargetException, ServletException;
    }

    private void returnParams(HttpServletRequest request, Map<String, Object> view) {
