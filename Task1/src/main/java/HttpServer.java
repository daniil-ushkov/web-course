...
class HttpServer {
    ...
    private void setResponse(Response response, File file) throws IOException {
    ...
    private String getHash(File content) throws IOException {
        return Integer.toString(Arrays.hashCode(readFile(content)));
    }

    private boolean containsIfNoneMatch(Request request) {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        File file = new File(root, URLDecoder.decode(request.getUri(),
                StandardCharsets.UTF_8.name()));
        File content;
        if (file.isFile()) {
            content = file;
        } else if (file.isDirectory()) {
            content = new File(file, "index.html");
        } else {
            content = new File(root, "index.html");
        }
        if (!containsIfNoneMatch(request)) {
            setResponse(response, content);
            return;
        }
        if (request.getHeader("If-None-Match").equals(getHash(content))) {
